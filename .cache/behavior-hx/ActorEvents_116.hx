...
class ActorEvents_116 extends ActorScript
{
	...
	override public function init()
	{
		...
		addKeyStateListener("Action 1", function(pressed:Bool, released:Bool, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && pressed)
			{
				if((_transformed == true))
				{
					...
					{
						_draw = false;
						Engine.engine.setGameAttribute("cornPicked", true);
						recycleActor(actor);
					}, actor);
				}
				else
		...
		/* ========================= Type & Type ========================== */
		addSceneCollisionListener(getActorType(116).ID, getActorType(0).ID, function(event:Collision, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				_colliding = true;
